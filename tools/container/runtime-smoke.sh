...
if inspect_container_image missing:latest \
    >"${SMOKE_DIR}/missing.stdout" \
    2>"${SMOKE_DIR}/missing.stderr"; then
...
if inspect_container_image available:latest \
    >"${SMOKE_DIR}/unavailable.stdout" \
    2>"${SMOKE_DIR}/unavailable.stderr"; then
