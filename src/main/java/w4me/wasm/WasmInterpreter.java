...
/** Provides the WASM interpreter implementation. */
public final class WasmInterpreter {
    ...
    /** Creates a new WASM interpreter. */
    ...
    /** Performs the instructions executed operation. */
    public long instructionsExecuted() {
        return instructionsExecuted;
    }

    /** Performs the dispatches executed operation. */
    public long dispatchesExecuted() {
        return dispatchesExecuted;
    }

    /** Performs the fast path calls operation. */
    public int fastPathCalls() {
        return 0;
    }

    /** Performs the compact block calls operation. */
    public int compactBlockCalls() {
        return compactBlockCalls;
    }

    /** Performs the compact instructions executed operation. */
    public int compactInstructionsExecuted() {
        return compactInstructionsExecuted;
    }

    /** Performs the trace loop calls operation. */
    public int traceLoopCalls() {
        return traceLoopCalls;
    }

    /** Performs the trace loop iterations operation. */
    public int traceLoopIterations() {
        return traceLoopIterations;
    }

    /** Performs the direct branch fast path int count operation. */
    ...
    /** Performs the direct branch fast path array count operation. */
    ...
    /** Updates the fast paths enabled. */
    ...
    /** Updates the direct numeric intrinsics enabled. */
    ...
    /** Updates the integer compact opcodes enabled. */
    ...
    public void setProfilingEnabled(boolean enabled) {
        ...
        if (enabled && opcodeCounts == null) {
            ...
            compactProfileCandidateDispatchLengths = new long[PROFILE_MAX_COMPACT_DISPATCHES + 1];
            compactProfileAcceptedDispatchLengths = new long[PROFILE_MAX_COMPACT_DISPATCHES + 1];
            compactProfileCandidateInstructionLengths = new long[PROFILE_MAX_COMPACT_INSTRUCTIONS + 1];
            compactProfileAcceptedInstructionLengths = new long[PROFILE_MAX_COMPACT_INSTRUCTIONS + 1];
        }
        if (enabled) {
            prepareCompactProfileMetadata();
        }
    ...
    /** Performs the opcode profile limit operation. */
    public int opcodeProfileLimit() {
        return PROFILE_OPCODE_LIMIT;
    }

    /** Performs the opcode count operation. */
    ...
    /** Performs the function call count operation. */
    public long functionCallCount(int functionIndex) {
        if (functionCallCounts == null || functionIndex < 0 || functionIndex >= functionCallCounts.length) {
            return 0;
        }
        return functionCallCounts[functionIndex];
    }

    /** Performs the function dispatch count operation. */
    public long functionDispatchCount(int functionIndex) {
        if (functionDispatchCounts == null || functionIndex < 0 || functionIndex >= functionDispatchCounts.length) {
            return 0;
        }
        return functionDispatchCounts[functionIndex];
    }

    /** Performs the opcode pair count operation. */
    ...
    /** Performs the compact profile accepted dispatch length count operation. */
    ...
    /** Performs the compact profile candidate instruction length count operation. */
    ...
    /** Performs the compact profile accepted instruction length count operation. */
    ...
    /** Invokes the cartridge lifecycle. */
    ...
    private void callFunction(int functionIndex) {
        ...
        if (body != null) {
            ...
            if (body.intrinsic == WasmModule.INTRINSIC_F32_FLOOR || body.intrinsic == WasmModule.INTRINSIC_F32_SIN) {
                ...
                int argument = popI32();
                pushI32(
                        body.intrinsic == WasmModule.INTRINSIC_F32_FLOOR
        ...
        if (body == null) {
            ...
            if (numericHostImportDispatchEnabled) {
                result = host.invoke(imported.hostId, values, argumentBase, argumentCount, module);
            } else {
                result = host.invoke(imported.module, imported.name, values, argumentBase, argumentCount, module);
            }
    ...
    private void execute(
        ...
        boolean compactEligible = compactExecutorEnabled
        ...
        try {
            ...
            while (pc < instructionCount) {
                ...
                if ((!InterpreterBuildConfig.RESIDENT_CODE_FAST_PATH || !residentCode)
                        && (codeOffset < codePageBase || codeOffset + 2 >= codePageBase + code.length)) {
                ...
                final int opcode = instruction & 0xffff;
                int auxiliary;
                ...
                switch (opcode) {
                    ...
                    case 0x0c:
                        if (InterpreterBuildConfig.DIRECT_BRANCH_FAST_PATH) {
                            ...
                            if (directBranchSite >= 0) {
                                int directBranchTarget = branchTargetBySite[directBranchSite];
                                int directBranchArity = branchArityBySite[directBranchSite];
                                int directBranchDestination = functionStackBase + branchHeightBySite[directBranchSite];
                                if (directBranchTarget >= 0
                                        && directBranchArity <= 1
                                        && directBranchDestination + directBranchArity <= valueTop) {
                                    if (directBranchArity != 0) {
                                        values[directBranchDestination] = values[valueTop - 1];
                                    }
                                    valueTop = directBranchDestination + directBranchArity;
                                    controlTop = functionControlBase + branchControlBySite[directBranchSite];
                        ...
                        if (InterpreterBuildConfig.INLINE_BRANCH_FAST_PATH) {
                            int inlineBranchAvailable = controlTop - functionControlBase;
                            if (operand >= 0 && operand < inlineBranchAvailable) {
                                int inlineBranchTarget = controlTop - 1 - operand;
                                int inlineBranchWord = controlWord[inlineBranchTarget];
                                boolean inlineBranchLoop = (inlineBranchWord & 0xffff) == WasmModule.LOOP;
                                int inlineBranchArity =
                                        inlineBranchLoop ? (inlineBranchWord >>> 16) & 0xff : inlineBranchWord >>> 24;
                                int inlineBranchDestination = controlBase[inlineBranchTarget];
                                if (inlineBranchArity <= 1 && inlineBranchDestination + inlineBranchArity <= valueTop) {
                                    if (inlineBranchArity != 0) {
                                        values[inlineBranchDestination] = values[valueTop - 1];
                                    }
                                    valueTop = inlineBranchDestination + inlineBranchArity;
                                    if (inlineBranchLoop) {
                        ...
                        if (InterpreterBuildConfig.DESCRIPTOR_SHADOW) {
                            ...
                            pc = branch(operand, functionStackBase, functionType.results.length, functionControlBase);
                        }
                        if (pc < 0) {
                            return;
                        }
                        ...
                        if (popI32() != 0) {
                            if (InterpreterBuildConfig.DIRECT_BRANCH_FAST_PATH) {
                                ...
                                if (takenBranchSite >= 0) {
                                    int takenBranchTarget = branchTargetBySite[takenBranchSite];
                                    int takenBranchArity = branchArityBySite[takenBranchSite];
                                    int takenBranchDestination =
                                            functionStackBase + branchHeightBySite[takenBranchSite];
                                    if (takenBranchTarget >= 0
                                            && takenBranchArity <= 1
                                            && takenBranchDestination + takenBranchArity <= valueTop) {
                                        if (takenBranchArity != 0) {
                                            values[takenBranchDestination] = values[valueTop - 1];
                                        }
                                        valueTop = takenBranchDestination + takenBranchArity;
                                        controlTop = functionControlBase + branchControlBySite[takenBranchSite];
                            ...
                            if (InterpreterBuildConfig.INLINE_BRANCH_FAST_PATH) {
                                int inlineTakenAvailable = controlTop - functionControlBase;
                                if (operand >= 0 && operand < inlineTakenAvailable) {
                                    int inlineTakenTarget = controlTop - 1 - operand;
                                    int inlineTakenWord = controlWord[inlineTakenTarget];
                                    boolean inlineTakenLoop = (inlineTakenWord & 0xffff) == WasmModule.LOOP;
                                    int inlineTakenArity =
                                            inlineTakenLoop ? (inlineTakenWord >>> 16) & 0xff : inlineTakenWord >>> 24;
                                    int inlineTakenDestination = controlBase[inlineTakenTarget];
                                    if (inlineTakenArity <= 1
                                            && inlineTakenDestination + inlineTakenArity <= valueTop) {
                                        if (inlineTakenArity != 0) {
                                            values[inlineTakenDestination] = values[valueTop - 1];
                                        }
                                        valueTop = inlineTakenDestination + inlineTakenArity;
                                        if (inlineTakenLoop) {
                            ...
                            if (InterpreterBuildConfig.DESCRIPTOR_SHADOW) {
                                ...
                                pc = branch(
                                        operand, functionStackBase, functionType.results.length, functionControlBase);
                            }
                            if (pc < 0) {
                                return;
                            }
                    ...
                    case 0x11:
                        ...
                        WasmModule.FuncType actualIndirectType = module.functionTypes[indirectFunction];
                        WasmModule.FuncType expectedIndirectType = module.types[operand];
                        if (actualIndirectType.canonicalId != expectedIndirectType.canonicalId) {
                    ...
                    case 0x1c:
                        ...
                        int genericSelectCondition = (int)
                                values[--valueTop]; // NOPMD -- Cursor mutation stays adjacent to the access to preserve
                        // compact Java 1.3 bytecode.
                        long genericSelectSecond =
                                values[--valueTop]; // NOPMD -- Cursor mutation stays adjacent to the access to preserve
                        // compact Java 1.3 bytecode.
                    ...
                    case 0x2b:
                        ...
                        break;
                    case 0x2c:
                        pushI32(module.memory[address(operand, 1)]);
                        pc++;
                        break;
                    case 0x2d:
                        if (valueTop <= 0) {
                        ...
                        values[valueTop++] = // NOPMD -- Compact Java 1.3 cursor bytecode.
                    ...
                    case 0x2f:
                        ...
                        break;
                    case 0x30:
                        push(module.memory[address(operand, 1)]);
                        pc++;
                        break;
                    case 0x31:
                    ...
                    case 0x45:
                        ...
                        values[valueTop - 1] = (int) values[valueTop - 1] == 0 ? 1 : 0;
                        pc++;
                        break;
                    case 0x46:
                    case 0x47:
                    ...
                    case 0x4f:
                        ...
                        int genericI32CompareRight = (int)
                                values[--valueTop]; // NOPMD -- Cursor mutation stays adjacent to the access to preserve
                        // compact Java 1.3 bytecode.
                        ...
                        switch (opcode) {
                            case 0x46:
                                genericI32Comparison = genericI32CompareLeft == genericI32CompareRight;
                                break;
                            case 0x47:
                                genericI32Comparison = genericI32CompareLeft != genericI32CompareRight;
                                break;
                            ...
                            case 0x49:
                                genericI32Comparison =
                                        (genericI32CompareLeft & 0xffffffffL) < (genericI32CompareRight & 0xffffffffL);
                                break;
                            case 0x4a:
                                genericI32Comparison = genericI32CompareLeft > genericI32CompareRight;
                                break;
                        ...
                        values[valueTop - 1] = genericI32Comparison ? 1 : 0;
                        pc++;
                        break;
                    case 0x50:
                        pushI32(pop() == 0 ? 1 : 0);
                    ...
                    case 0x6b:
                        ...
                        int genericSubtractRight = (int)
                                values[--valueTop]; // NOPMD -- Cursor mutation stays adjacent to the access to preserve
                        // compact Java 1.3 bytecode.
                        values[valueTop - 1] = (long) ((int) values[valueTop - 1] - genericSubtractRight);
                        pc++;
                        break;
                    case 0x6c:
                        pushI32(popI32Second() * popI32First());
                    ...
                    case 0x72:
                        ...
                        int genericOrRight = (int)
                                values[--valueTop]; // NOPMD -- Cursor mutation stays adjacent to the access to preserve
                        // compact Java 1.3 bytecode.
                        values[valueTop - 1] = (int) values[valueTop - 1] | genericOrRight;
                        pc++;
                        break;
                    case 0x73:
                        pushI32(popI32Second() ^ popI32First());
                    ...
                    case 0x74:
                        ...
                        int genericShiftLeft = (int)
                                values[--valueTop]; // NOPMD -- Cursor mutation stays adjacent to the access to preserve
                        // compact Java 1.3 bytecode.
                        values[valueTop - 1] = (long) ((int) values[valueTop - 1] << (genericShiftLeft & 31));
                        pc++;
                        break;
                    case 0x75:
                    ...
                    case 0x77:
                        ...
                        pushI32((i32RotateLeftValue << i32RotateLeft) | (i32RotateLeftValue >>> (32 - i32RotateLeft)));
                        pc++;
                        break;
                    case 0x78:
                    ...
                    case 0x89:
                        ...
                        push((i64RotateLeftValue << i64RotateLeft) | (i64RotateLeftValue >>> (64 - i64RotateLeft)));
                        pc++;
                        break;
                    case 0x8a:
                        int i64RotateRight = (int) pop() & 63;
                        long i64RotateRightValue = pop();
                        push((i64RotateRightValue >>> i64RotateRight) | (i64RotateRightValue << (64 - i64RotateRight)));
                        pc++;
                        break;
                    case 0x8b:
                    case 0x8c:
                    ...
                    case WasmModule.W4IR_LOCAL_LOCAL_F32_MUL + WasmModule.W4IR_EXECUTION_OFFSET:
                        auxiliary = code[pageOffset + 2];
                        pushI32(Float.floatToIntBits(Float.intBitsToFloat((int) locals[operand])
                    ...
                    case WasmModule.W4IR_LOCAL_F32_CONST_MUL + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        pushI32(Float.floatToIntBits(
                                Float.intBitsToFloat((int) locals[operand]) * Float.intBitsToFloat(auxiliary)));
                        executed += 2;
                        pc += 3;
                        break;
                    ...
                    case WasmModule.W4IR_LOCAL_TEE_F32_MUL_ADD + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        pushI32(Float.floatToIntBits(teeAddLeft + teeMultiplyLeft * teeMultiplyRight));
                        executed += 2;
                        pc += 3;
                        break;
                    case WasmModule.W4IR_LOCAL_F32_MUL_ADD + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        pushI32(Float.floatToIntBits(localAddLeft + localMultiplyLeft * localMultiplyRight));
                        executed += 2;
                        pc += 3;
                        break;
                    case WasmModule.W4IR_LOCAL_LOCAL_F32_MUL_ADD + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        pushI32(Float.floatToIntBits(localsAddLeft + localsMultiplyLeft * localsMultiplyRight));
                        executed += 3;
                        pc += 4;
                        break;
                    case WasmModule.W4IR_LOCAL_I32_CONST_EQ_BR_IF + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        if ((int) locals[compareBranchLocal] == operand) {
                            if (InterpreterBuildConfig.DESCRIPTOR_SHADOW) {
                                ...
                                pc = branch(
                                        auxiliary, functionStackBase, functionType.results.length, functionControlBase);
                            }
                            if (pc < 0) {
                                return;
                            }
                    ...
                    case WasmModule.W4IR_LOCAL_TEE_MUL_ADD_SET_LOCAL_LOCAL + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        int teeCombinedResult = Float.floatToIntBits(
                                teeCombinedAddLeft + teeCombinedMultiplyLeft * teeCombinedMultiplyRight);
                    ...
                    case WasmModule.W4IR_LOCAL_MUL_ADD_SET + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        float localSetMultiplyRight = Float.intBitsToFloat((int) locals[localSetMultiplySource]);
                        float localSetMultiplyLeft = Float.intBitsToFloat(popI32());
                        float localSetAddLeft = Float.intBitsToFloat(popI32());
                        int localSetResult =
                                Float.floatToIntBits(localSetAddLeft + localSetMultiplyLeft * localSetMultiplyRight);
                    ...
                    case WasmModule.W4IR_F32_LOAD_LOCAL_MUL_ADD + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        float loadMultiplyLeft = Float.intBitsToFloat(loadI32(loadMultiplyAddress));
                        float loadMultiplyRight = Float.intBitsToFloat((int) locals[auxiliary]);
                        float loadAddLeft = Float.intBitsToFloat(popI32());
                        pushI32(Float.floatToIntBits(loadAddLeft + loadMultiplyLeft * loadMultiplyRight));
                        executed += 3;
                        pc += 4;
                        break;
                    ...
                    case WasmModule.W4IR_F32_DIV_TEE_MUL_ADD_SET_LOCAL_LOCAL + WasmModule.W4IR_EXECUTION_OFFSET:
                        auxiliary = code[pageOffset + 2];
                        executeF32DivTeeMulAddSetLocalLocal(instruction, operand, auxiliary, locals);
                        executed += 6;
                        pc += 7;
                        break;
                    ...
                    case WasmModule.W4IR_TRIPLE_F32_STORE_LOCAL_LOAD_LOCAL + WasmModule.W4IR_EXECUTION_OFFSET:
                        auxiliary = code[pageOffset + 2];
                        executeTripleF32StoreLocalLoadLocal(instruction, operand, auxiliary, locals);
                        executed += 12;
                        pc += 13;
                        break;
                    case WasmModule.W4IR_LOCAL_TEE_MUL_ADD_SET_LOAD_MUL_ADD + WasmModule.W4IR_EXECUTION_OFFSET:
                        auxiliary = code[pageOffset + 2];
                        executeLocalTeeMulAddSetLoadMulAdd(instruction, operand, auxiliary, locals);
                        executed += 9;
                        pc += 10;
                        break;
                    ...
                    case WasmModule.W4IR_F32_MUL_CONST + WasmModule.W4IR_EXECUTION_OFFSET:
                        pushI32(Float.floatToIntBits(Float.intBitsToFloat(popI32()) * Float.intBitsToFloat(operand)));
                        executed++;
                        pc += 2;
                        break;
                    ...
                    case WasmModule.W4IR_LOCAL_I32_CONST_ADD_SET + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        while (true) {
                            ...
                            locals[addTarget] = (long) ((int) locals[addSource] + auxiliary);
                            executed += 3;
                            pc += 4;
                            ...
                            if (nextAddOffset < codePageBase || nextAddOffset + 2 >= codePageBase + code.length) {
                    ...
                    case WasmModule.W4IR_LOCAL_LOCAL_F32_STORE + WasmModule.W4IR_EXECUTION_OFFSET:
                        ...
                        while (true) {
                            ...
                            if (nextStoreOffset < codePageBase || nextStoreOffset + 2 >= codePageBase + code.length) {
                    ...
                    default:
                        throw new WasmTrap(
                                "opcode not implemented: 0x" + Integer.toHexString(WasmModule.originalOpcode(opcode)));
                }
            }
    ...
    private void profileInstruction(
            int functionIndex, int pc, int previousPreviousOpcode, int previousOpcode, int opcode) {
        functionDispatchCounts[functionIndex]++;
        opcodeCounts[opcode]++;
        if (previousOpcode >= 0) {
            opcodePairCounts.add(pairKey(previousOpcode, opcode));
            if (previousPreviousOpcode >= 0) {
                opcodeTripleCounts.add(tripleKey(previousPreviousOpcode, previousOpcode, opcode));
            }
        }
    ...
    private void profileCompactCandidate(int functionIndex, int pc) {
        ...
        final int dispatchLength = dispatchLengths[pc] & 0xff;
        final int instructionLength = compactProfileInstructionLengths[functionIndex][pc] & 0xffff;
        ...
        compactProfileCandidateInstructionLengths[boundedProfileInstructionLength(instructionLength)]++;
        if (compactBlockEnds[functionIndex][pc] > pc) {
            ...
            compactProfileAcceptedInstructionLengths[boundedProfileInstructionLength(instructionLength)]++;
        } else {
    ...
    private void prepareCompactProfileMetadata() {
        int functionIndex;
        for (functionIndex = module.imports.length; functionIndex < module.functions.length; functionIndex++) {
            WasmModule.FunctionBody body = module.functions[functionIndex];
            ...
            compactBlockEnds[functionIndex] =
                    buildCompactBlockEnds(body, dispatchLengths, instructionLengths, reasons, breakOpcodes);
    ...
    private int[] compactBlockEnds(int functionIndex, WasmModule.FunctionBody body) {
    ...
    private int[] buildCompactBlockEnds(
        ...
        for (pc = 0; pc < instructionCount; pc++) {
            ...
            int opcode = WasmModule.originalOpcode(body.code[offset] & 0xffff);
            if (opcode == WasmModule.LOOP && pc + 1 < instructionCount) {
                branchTargets[pc + 1] = true;
            }
            if (opcode == WasmModule.BLOCK || opcode == WasmModule.LOOP || opcode == WasmModule.IF) {
        ...
        for (pc = 0; pc < instructionCount; pc++) {
            ...
            while (true) {
                ...
                if (dispatchCount >= PROFILE_MAX_COMPACT_DISPATCHES) {
                    ...
                    breakOpcode = WasmModule.originalOpcode(body.code[cursor * WasmModule.W4IR_STRIDE] & 0xffff);
                    break;
                }
                ...
                int executionOpcode = body.code[cursor * WasmModule.W4IR_STRIDE] & 0xffff;
            ...
            if (profileDispatchLengths != null) {
                ...
                profileBreakOpcodes[pc] = (char) (breakOpcode < 0 ? 0xffff : breakOpcode);
            }
        }
        return ends;
    }
    ...
    private static int boundedProfileInstructionLength(int length) {
        return length > PROFILE_MAX_COMPACT_INSTRUCTIONS ? PROFILE_MAX_COMPACT_INSTRUCTIONS : length;
    }
    ...
    private static long tripleKey(int firstOpcode, int secondOpcode, int thirdOpcode) {
        return ((long) firstOpcode << 32) | ((long) secondOpcode << 16) | (long) thirdOpcode;
    }
    ...
    private boolean isCompactOpcode(int opcode) {
        if ((opcode >= WasmModule.EXECUTION_W4IR_FIRST
                        && opcode <= WasmModule.W4IR_LOCAL_LOCAL_F32_STORE + WasmModule.W4IR_EXECUTION_OFFSET)
                || (opcode >= WasmModule.W4IR_LOCAL_TEE_MUL_ADD_SET_LOCAL_LOCAL + WasmModule.W4IR_EXECUTION_OFFSET
                        && opcode <= WasmModule.W4IR_F32_LOAD_LOCAL_MUL_ADD + WasmModule.W4IR_EXECUTION_OFFSET)
                || (opcode >= WasmModule.W4IR_LOCAL_SET_ADD_SET + WasmModule.W4IR_EXECUTION_OFFSET
                        && opcode <= WasmModule.W4IR_F32_LOAD_LOCAL_MUL_ADD_SET + WasmModule.W4IR_EXECUTION_OFFSET)
                || (opcode >= WasmModule.W4IR_LOCAL_TRIPLE_F32_STORE + WasmModule.W4IR_EXECUTION_OFFSET
                        && opcode <= WasmModule.W4IR_LOCAL_SET_DUAL_ADD_SET + WasmModule.W4IR_EXECUTION_OFFSET)
                || opcode == WasmModule.W4IR_I32_LOAD_LOCAL_TEE + WasmModule.W4IR_EXECUTION_OFFSET) {
            return true;
        }
    ...
    private void executeCompactBlock(int[] code, int pc, int end, long[] locals) {
        while (pc < end) {
            ...
            int span = 1;
            boolean accountAfterHandler = opcode >= WasmModule.EXECUTION_W4IR_FIRST
                    && opcode < WasmModule.W4IR_I32_LOAD_LOCAL_TEE + WasmModule.W4IR_EXECUTION_OFFSET;
            ...
            switch (opcode) {
                case 0x1b:
                    ...
                    int selectCondition = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    long selectSecond = values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    long selectFirst = values[valueTop - 1];
                    values[valueTop - 1] = selectCondition != 0 ? selectFirst : selectSecond;
                    break;
                case 0x20:
                    if (valueTop >= values.length) {
                        throw new WasmTrap("value stack exhausted");
                    }
                    values[valueTop++] = locals[operand]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    break;
                case 0x21:
                    if (valueTop <= 0) {
                        throw new WasmTrap("value stack underflow");
                    }
                    locals[operand] = values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    break;
                case 0x22:
                    if (valueTop <= 0) {
                        throw new WasmTrap("value stack underflow");
                ...
                case 0x28:
                    ...
                    int loadI32Address = checkedAddress((int) values[valueTop - 1], operand, 4);
                ...
                case WasmModule.W4IR_I32_LOAD_LOCAL_TEE + WasmModule.W4IR_EXECUTION_OFFSET:
                    ...
                    int fusedLoadAddress = checkedAddress((int) values[valueTop - 1], operand, 4);
                ...
                case 0x2a:
                    ...
                    int loadF32Address = checkedAddress((int) values[valueTop - 1], operand, 4);
                ...
                case 0x4f:
                    ...
                    int compactI32Right = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    int compactI32Left = (int) values[valueTop - 1];
                    boolean compactI32Comparison;
                    switch (opcode) {
                        case 0x46:
                            compactI32Comparison = compactI32Left == compactI32Right;
                            break;
                        case 0x47:
                            compactI32Comparison = compactI32Left != compactI32Right;
                            break;
                        case 0x48:
                            ...
                            break;
                        case 0x49:
                            compactI32Comparison = (compactI32Left & 0xffffffffL) < (compactI32Right & 0xffffffffL);
                            break;
                        case 0x4a:
                            compactI32Comparison = compactI32Left > compactI32Right;
                            break;
                    ...
                    values[valueTop - 1] = compactI32Comparison ? 1 : 0;
                    break;
                case 0x5d:
                case 0x5e:
                case 0x60:
                ...
                case 0x6a:
                    ...
                    int addRight = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    values[valueTop - 1] = (long) ((int) values[valueTop - 1] + addRight);
                    break;
                case 0x6b:
                    if (valueTop < 2) {
                        throw new WasmTrap("value stack underflow");
                    }
                    int subtractRight = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    values[valueTop - 1] = (long) ((int) values[valueTop - 1] - subtractRight);
                    break;
                case 0x6c:
                    if (valueTop < 2) {
                        throw new WasmTrap("value stack underflow");
                    }
                    int multiplyRightI32 = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    values[valueTop - 1] = (long) ((int) values[valueTop - 1] * multiplyRightI32);
                    break;
                case 0x71:
                    if (valueTop < 2) {
                        throw new WasmTrap("value stack underflow");
                    }
                    int andRight = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                ...
                case 0x72:
                    ...
                    int orRight = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                ...
                case 0x73:
                    ...
                    int xorRight = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                ...
                case 0x74:
                    ...
                    int shiftLeft = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    values[valueTop - 1] = (long) ((int) values[valueTop - 1] << (shiftLeft & 31));
                    break;
                case 0x75:
                    if (valueTop < 2) {
                        throw new WasmTrap("value stack underflow");
                    }
                    int shiftRightSigned = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    values[valueTop - 1] = (int) values[valueTop - 1] >> (shiftRightSigned & 31);
                    break;
                case 0x76:
                    if (valueTop < 2) {
                        throw new WasmTrap("value stack underflow");
                    }
                    int shiftRightUnsigned = (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
                    values[valueTop - 1] = (long) ((int) values[valueTop - 1] >>> (shiftRightUnsigned & 31));
                    break;
                case 0x84:
                    push(popSecond() | popFirst());
                    break;
                ...
                case 0x95:
                    ...
                    float compactF32Right = Float.intBitsToFloat(
                            (int) values[--valueTop]); // NOPMD -- Compact Java 1.3 cursor bytecode.
                    float compactF32Left = Float.intBitsToFloat((int) values[valueTop - 1]);
                ...
                case 0xbe:
                    break;
                default:
                    span = executeCompactFused(opcode, instruction, operand, auxiliary, locals);
                    break;
            }
            if (accountAfterHandler) {
    ...
    private int executeCompactFused(int opcode, int instruction, int operand, int auxiliary, long[] locals) {
        switch (opcode) {
            case WasmModule.W4IR_LOCAL_LOCAL_F32_MUL + WasmModule.W4IR_EXECUTION_OFFSET:
                pushI32(Float.floatToIntBits(
                        Float.intBitsToFloat((int) locals[operand]) * Float.intBitsToFloat((int) locals[auxiliary])));
                return 3;
            case WasmModule.W4IR_LOCAL_F32_CONST_MUL + WasmModule.W4IR_EXECUTION_OFFSET:
                pushI32(Float.floatToIntBits(
                        Float.intBitsToFloat((int) locals[operand]) * Float.intBitsToFloat(auxiliary)));
            ...
            case WasmModule.W4IR_F32_CONST_MUL_ADD + WasmModule.W4IR_EXECUTION_OFFSET:
                ...
                float constantProduct = constantMultiplyRight * Float.intBitsToFloat(operand);
            ...
            case WasmModule.W4IR_LOCAL_LOCAL_F32_LOAD + WasmModule.W4IR_EXECUTION_OFFSET:
                ...
                push(loadI32(checkedAddress((int) locals[loadAddress], auxiliary, 4)) & 0xffffffffL);
                return 3;
            case WasmModule.W4IR_LOCAL_TEE_F32_MUL_ADD + WasmModule.W4IR_EXECUTION_OFFSET:
                locals[operand] = peek();
            ...
            case WasmModule.W4IR_LOCAL_LOCAL_F32_STORE + WasmModule.W4IR_EXECUTION_OFFSET:
                ...
                storeI32(checkedAddress((int) locals[storeAddress], auxiliary, 4), (int) locals[storeValue]);
                return 3;
            case WasmModule.W4IR_LOCAL_TEE_MUL_ADD_SET_LOCAL_LOCAL + WasmModule.W4IR_EXECUTION_OFFSET:
                ...
                int combinedResult = Float.floatToIntBits(combinedAdd + combinedLeft * combinedRight);
            ...
            case WasmModule.W4IR_LOCAL_MUL_ADD_SET + WasmModule.W4IR_EXECUTION_OFFSET:
                ...
                int multiplyResult = Float.floatToIntBits(multiplyAdd + multiplyLeft * multiplyRight);
            ...
            case WasmModule.W4IR_F32_SIN_INTRINSIC + WasmModule.W4IR_EXECUTION_OFFSET:
                executeDirectNumericIntrinsic(
                        operand, opcode == WasmModule.W4IR_F32_SIN_INTRINSIC + WasmModule.W4IR_EXECUTION_OFFSET);
                return 1;
            default:
                throw new WasmTrap("unsupported compact opcode: 0x" + Integer.toHexString(opcode));
        }
    }
    ...
    private int executeLocalAddSetBranch(
        ...
        locals[target] = (long) ((int) locals[source] + auxiliary);
        return branch(instruction >>> 16, functionStackBase, functionResultCount, functionControlBase);
    }
    ...
    private void executeLocalSetAddSet(int instruction, int operand, int auxiliary, long[] locals) {
        int firstTarget = instruction >>> 16;
        int source = operand >>> 16;
        int secondTarget = operand & 0xffff;
        locals[firstTarget] = pop();
        locals[secondTarget] = (long) ((int) locals[source] + auxiliary);
    }

    private void executeLocalSetLocalLocalF32Load(int instruction, int operand, int auxiliary, long[] locals) {
    ...
    private void executeLocalLocalF32LoadLocal(int instruction, int operand, int auxiliary, long[] locals) {
    ...
    private void executeF32DivTeeMulAddSetLocalLocal(int instruction, int operand, int auxiliary, long[] locals) {
    ...
    private void executeF32LoadLocalMulAddSet(int instruction, int operand, int auxiliary, long[] locals) {
        int setTarget = instruction >>> 16;
    ...
    private void executeLocalTripleF32Store(int instruction, int operand, int auxiliary, long[] locals) {
        ...
        storeI32(checkedAddress(baseAddress, baseOffset, 4), (int) locals[firstValueLocal]);
        storeI32(checkedAddress(baseAddress, baseOffset + 4, 4), (int) locals[secondValueLocal]);
        storeI32(checkedAddress(baseAddress, baseOffset + 8, 4), (int) locals[thirdValueLocal]);
    }

    private void executeLocal4F32MulAddTee(int instruction, int operand, int auxiliary, long[] locals) {
        ...
        int firstProductBits = Float.floatToIntBits(
                Float.intBitsToFloat((int) locals[firstSource]) * Float.intBitsToFloat((int) locals[secondSource]));
        float firstProduct = Float.intBitsToFloat(firstProductBits);
        float secondProduct =
                Float.intBitsToFloat((int) locals[thirdSource]) * Float.intBitsToFloat((int) locals[fourthSource]);
        int result = Float.floatToIntBits(firstProduct + secondProduct);
        locals[teeTarget] = result & 0xffffffffL;
        pushI32(result);
    }

    private void executeF32LoadNegIndexDiv(int instruction, int operand, int auxiliary, long[] locals) {
        ...
        int index = (int) locals[firstSource] | (((int) locals[secondSource] & mask) << shift);
    ...
    private void executeTripleF32StoreLocalLoadLocal(int instruction, int operand, int auxiliary, long[] locals) {
        ...
        final int firstSource = (operand >>> 8) & 0xff;
        final int loadAddressLocal = operand & 0xff;
        final int thirdSource = auxiliary >>> 24;
        int storeOffset = (auxiliary >>> 16) & 0xff;
        final int loadOffset = (auxiliary >>> 8) & 0xff;
        int storeBase = (int) locals[storeAddressLocal];
        storeI32(checkedAddress(storeBase, storeOffset, 4), (int) locals[firstValueLocal]);
        storeI32(checkedAddress(storeBase, storeOffset + 4, 4), (int) locals[secondValueLocal]);
        storeI32(checkedAddress(storeBase, storeOffset + 8, 4), (int) locals[thirdValueLocal]);
        push(locals[firstSource]);
    ...
    private void executeLocalTeeMulAddSetLoadMulAdd(int instruction, int operand, int auxiliary, long[] locals) {
        int packedTargets = instruction >>> 16;
        ...
        int firstResult = Float.floatToIntBits(firstAdd + firstMultiplyLeft * firstMultiplyRight);
        ...
        pushI32(Float.floatToIntBits(secondAdd + secondMultiplyLeft * secondMultiplyRight));
    }

    private void executeLocalSetDualAddSet(int instruction, int operand, int auxiliary, long[] locals) {
        ...
        locals[firstTarget] = pop();
        locals[secondTarget] = (long) ((int) locals[firstSource] + firstConstant);
        locals[thirdTarget] = (long) ((int) locals[secondSource] + secondConstant);
    }
    ...
    private int executeCountedF32Trace(
        ...
        final int storeOperand = w4irWord(body, storePc, 1);
        final int storeAuxiliary = w4irWord(body, storePc, 2);
        int divideInstruction = w4irWord(body, dividePc, 0);
        final int divideOperand = w4irWord(body, dividePc, 1);
        final int divideAuxiliary = w4irWord(body, dividePc, 2);
        int multiplyInstruction = w4irWord(body, multiplyPc, 0);
        final int multiplyOperand = w4irWord(body, multiplyPc, 1);
        final int multiplyAuxiliary = w4irWord(body, multiplyPc, 2);
        int addInstruction = w4irWord(body, addPc, 0);
        final int addOperand = w4irWord(body, addPc, 1);
        final int addAuxiliary = w4irWord(body, addPc, 2);
        int backedgeInstruction = w4irWord(body, backedgePc, 0);
        final int backedgeOperand = w4irWord(body, backedgePc, 1);
        final int backedgeAuxiliary = w4irWord(body, backedgePc, 2);
        ...
        while ((int) locals[compareLocal] != limit) {
            ...
            executeTripleF32StoreLocalLoadLocal(storeInstruction, storeOperand, storeAuxiliary, locals);
            instructionsExecuted += 12;

            beginTraceInstruction();
            executeF32LoadNegIndexDiv(divideInstruction, divideOperand, divideAuxiliary, locals);
            instructionsExecuted += 10;

            beginTraceInstruction();
            executeLocalTeeMulAddSetLoadMulAdd(multiplyInstruction, multiplyOperand, multiplyAuxiliary, locals);
            instructionsExecuted += 9;

            beginTraceInstruction();
            executeLocalSetDualAddSet(addInstruction, addOperand, addAuxiliary, locals);
        ...
        return branch(exitDepth, functionStackBase, functionResultCount, functionControlBase);
    }
    ...
    private int executeBranchIfLocalI32Const(
            ...
            int functionControlBase) {
        if (popI32() != 0) {
            return branch(auxiliary, functionStackBase, functionResultCount, functionControlBase);
        }
    ...
    private int branchWithDescriptorShadow(
        ...
        verifyDescriptorAgainstLegacy(
                body, descriptorIndex, depth, functionStackBase, functionResultCount, functionControlBase);
        int offset = descriptorIndex * WasmModule.BRANCH_DESCRIPTOR_STRIDE;
        int destination =
                functionStackBase + body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_VALUE_HEIGHT];
        int arity = body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_ARITY];
        int expectedControlTop =
                functionControlBase + body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_CONTROL_DEPTH];
        int expectedPc = body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_TARGET_PC];
        int actualPc = branch(depth, functionStackBase, functionResultCount, functionControlBase);
        if (actualPc != expectedPc || valueTop != destination + arity || controlTop != expectedControlTop) {
    ...
    private void verifyDescriptorFallthrough(
        ...
        verifyDescriptorAgainstLegacy(
                body, descriptorIndex, depth, functionStackBase, functionResultCount, functionControlBase);
        if (valueTop != expectedValueTop || controlTop != expectedControlTop || actualPc != expectedPc) {
    ...
    private void verifyDescriptorAgainstLegacy(
            ...
            int functionControlBase) {
        int offset = descriptorIndex * WasmModule.BRANCH_DESCRIPTOR_STRIDE;
        if (descriptorIndex < 0 || offset > body.branchDescriptors.length - WasmModule.BRANCH_DESCRIPTOR_STRIDE) {
        ...
        if (depth == available) {
            ...
            activeControlDepth = loop ? target + 1 - functionControlBase : target - functionControlBase;
            flags = loop ? WasmModule.BRANCH_DESCRIPTOR_LOOP_TARGET : 0;
        }
        if (body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_TARGET_PC] != targetPc
                || body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_VALUE_HEIGHT] != destinationHeight
                || body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_ARITY] != arity
                || body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_CONTROL_DEPTH] != activeControlDepth
                || body.branchDescriptors[offset + WasmModule.BRANCH_DESCRIPTOR_FLAGS] != flags) {
            throw new WasmTrap("branch descriptor metadata mismatch");
        }
    }

    private int branch(int depth, int functionStackBase, int functionResultCount, int functionControlBase) {
        int available = controlTop - functionControlBase;
        if (depth == available) {
    ...
    private int checkedAddress(int base, int offset, int size) {
        ...
        if (base < 0 || offset < 0 || base > maximumBase || offset > maximumBase - base) {
            throw new WasmTrap("out-of-bounds memory access");
        }
        return base + offset;
    }
    ...
    private void executeBulkMemory(int opcode, int operand) {
        switch (opcode) {
            case WasmModule.EXECUTION_BULK_FIRST + 0x8:
                ...
                System.arraycopy(dataSegment, initializeSource, module.memory, initializeDestination, initializeLength);
                return;
            case WasmModule.EXECUTION_BULK_FIRST + 0x9:
            ...
            case WasmModule.EXECUTION_BULK_FIRST + 0xa:
                ...
                System.arraycopy(module.memory, copySource, module.memory, copyDestination, copyLength);
                return;
            default:
    ...
    private int countTrailingZerosI64(long value) {
        ...
        int low = (int) value;
        return low != 0 ? countTrailingZerosI32(low) : 32 + countTrailingZerosI32((int) (value >>> 32));
    }
    ...
    private void unaryF32(int opcode) {
        ...
        if (opcode == 0x8d) {
            ...
            result = (float) (value < 0.0f ? Math.ceil((double) value) : Math.floor((double) value));
        } else if (opcode == 0x90) {
    ...
    private double nearestF64(double value) {
        if (value != value || value == 0.0 || value >= 4503599627370496.0 || value <= -4503599627370496.0) {
            return value;
        }
    ...
    static strictfp int sinF32BitsForFastPath(int bits) {
        ...
        if (quadrantFloat < SIN_I32_THRESHOLD) {
            ...
            quadrantFloat = Float.intBitsToFloat(floorF32BitsStatic(Float.floatToIntBits(quadrantFloat)));
            quadrant = 0x80000000;
        }
        ...
        final boolean negate = (original < 0.0f) ^ (octant > 3);
    ...
    private void pushI32(int value) {
        try {
            values[valueTop++] = value; // NOPMD -- Compact Java 1.3 cursor bytecode.
        } catch (ArrayIndexOutOfBoundsException failure) {
            if (valueTop > values.length || valueTop == Integer.MIN_VALUE) {
                valueTop--;
                throw new WasmTrap("value stack exhausted"); // NOPMD -- CLDC 1.1 has no portable cause chaining.
            }
            throw failure;
        }
    }
    ...
    private long pop() {
        try {
            return values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
        } catch (ArrayIndexOutOfBoundsException failure) {
            valueTop++;
            if (valueTop <= 0) {
                throw new WasmTrap("value stack underflow"); // NOPMD -- CLDC 1.1 has no portable cause chaining.
            }
            valueTop--;
            throw failure;
        }
    ...
    private int popI32() {
        try {
            return (int) values[--valueTop]; // NOPMD -- Compact Java 1.3 cursor bytecode.
        } catch (ArrayIndexOutOfBoundsException failure) {
            valueTop++;
            if (valueTop <= 0) {
                throw new WasmTrap("value stack underflow"); // NOPMD -- CLDC 1.1 has no portable cause chaining.
